...
class ArrayOfItems {
	...
	/** Construit une liste d'items � partir d'une ligne de script brut */
	static public ArrayOfItems create(String line) throws ScriptException {
		...
		while(cur < len) {
			...
			/// Paranthesis \\\
				...
				// dernier �l�ment ou structure vide
				if(sl.isEmpty())
					throw new ScriptException("Nothing before the end of the parenthesis");
				
				sl.isInSubList = true;
				
				// completion de la liste m�re d'ArrayOfItems
...
public class ArrayOfItems {
	...
	static private ArrayList<Item> splitString(String s) throws ScriptException {
		...
		while(cur < len) {
			...
			/// Math Operators \\\ // TODO : fuck code pour chagement de mot cl�
			/*else if(c == '*') {
			...
			else if(c == '-') { // TODO conflit avec signe de n�gation
